  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      {localNombre&&visitanteNombre&&(<>
        ...
        {goleadores.map((g,i)=>{
          const gOpts=[...new Set([...(allSels.find(s=>s.country===g.equipo)?.squad||[]).map(p=>p.name),...Object.values(mundial?.stats||{}).filter(p=>p.goles>0&&(!g.equipo||p.selName===g.equipo)).map(p=>p.name)])].sort();
          return(<div key={i} style={{display:"flex",gap:5,alignItems:"center",marginBottom:4}}>
            ...
            {gOpts.length>0&&<select value={g.nombre} onChange={e=>{const n=[...goleadores];n[i]={...n[i],nombre:e.target.value};setGoleadores(n);}} style={{flex:1,padding:"6px 4px",borderRadius:7,border:`1px solid ${C.borderDark}`,background:C.card,color:g.nombre?C.text:C.textFaint,fontSize:10,fontFamily:"'DM Sans',sans-serif",outline:"none"}}><option value="">— Jugador —</option>{gOpts.map(nm=><option key={nm} value={nm}>{nm}</option>)}</select>}
            {gOpts.length===0&&<input value={g.nombre} onChange={e=>{const n=[...goleadores];n[i]={...n[i],nombre:e.target.value};setGoleadores(n);}} placeholder="Nombre" style={{flex:1,padding:"6px 8px",borderRadius:7,border:`1px solid ${C.borderDark}`,background:C.card,color:C.text,fontSize:10,fontFamily:"'DM Sans',sans-serif",outline:"none"}}/>}
        ...
        {asistencias.map((a,i)=>{
          const aOpts=[...new Set([...(allSels.find(s=>s.country===a.equipo)?.squad||[]).map(p=>p.name),...Object.values(mundial?.stats||{}).filter(p=>p.asistencias>0&&(!a.equipo||p.selName===a.equipo)).map(p=>p.name)])].sort();
          return(<div key={i} style={{display:"flex",gap:5,alignItems:"center",marginBottom:4}}>
            ...
            {aOpts.length>0&&<select value={a.nombre} onChange={e=>{const n=[...asistencias];n[i]={...n[i],nombre:e.target.value};setAsistencias(n);}} style={{flex:1,padding:"6px 4px",borderRadius:7,border:`1px solid ${C.borderDark}`,background:C.card,color:a.nombre?C.text:C.textFaint,fontSize:10,fontFamily:"'DM Sans',sans-serif",outline:"none"}}><option value="">— Jugador —</option>{aOpts.map(nm=><option key={nm} value={nm}>{nm}</option>)}</select>}
            {aOpts.length===0&&<input value={a.nombre} onChange={e=>{const n=[...asistencias];n[i]={...n[i],nombre:e.target.value};setAsistencias(n);}} placeholder="Nombre" style={{flex:1,padding:"6px 8px",borderRadius:7,border:`1px solid ${C.borderDark}`,background:C.card,color:C.text,fontSize:10,fontFamily:"'DM Sans',sans-serif",outline:"none"}}/>}
          ...
          <HomeScreen
            ...
            onSelect={comp=>{
              ...
              if(matched) setActiveLineupId(matched.id);
                ...
                // Create lineup for this comp if doesn't exist
